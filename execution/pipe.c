...
void exec_pipe(t_data *data, char **env)
{
	...
	while (i < data->n_of_cmds)
	{
		...
		if (pid < 0)
			...
			process1(fd,(t_data *)data->pipe_cmd, env);
		else if (pid == 0 && i == (data->n_of_cmds - 1))
			last_process(fd,(t_data *)data->pipe_cmd, env,i);
		else if (pid == 0 && (i % 2) != 0)
			process_in_middle_odd(fd,(t_data *)data->pipe_cmd, env);
		else if (pid == 0 && (i % 2) == 0)
			process_in_middle_even(fd,(t_data *)data->pipe_cmd, env);
